...
class RT_1d_simulations:
    def __init__(self):
        """
            ...
            To be assigned manually after the creation of the instance
    ...
    def bc_transport(self, D):
        """
        Set Dirichlet condition for all the Laguerre's coefficients.
        """
        D[:,0] = 0
        D[:,-1] = 0
        return
    ...
    def initialise(self):
        ...
        Give a value to the coefficients D0 and C0
        """
    ...
    def build_Am(self):
        """
        Construction of the big matrix of the system
        """
    ...
    def kinetic_iteration(self, tn, dt):
        """
        Execute one iteration in time for the run and tumble equation.
        """
        if tn == 0:
            self.build_Am()
    ...
    def diagnostic(self):
        """
        Save the state of the solution into files:
            f_tn.pkl    |   save for f
            rho_tn.pkl  |   save for rho
        
        Before saving, we need to compute the Laguerre polynomials
        and reconstruct f in the v variable.

        Modify the name of folder if needed.
        """
        dx = (self.xmax - self.xmin) / self.nx;
        x= np.linspace(self.xmin, self.xmax, self.nx +1)
